...
export default function GiftCard({ title, imageUrl, onSelect, className }: Props) {
  return (
    <button className={`border rounded p-3 text-center hover:bg-gray-50 focus:outline-none focus:ring ${className || ""}`} onClick={onSelect} aria-label={`Select gift ${title}`}>
      {imageUrl ? (
